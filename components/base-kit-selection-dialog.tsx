...
interface BaseKitSelectionDialogProps {
  currentBaseKit: Kit | null;
  onBaseKitSelect: (baseKit: Kit | null) => void;
  excludeKitId?: string; // Kit ID to exclude from search results (current kit)
}

export function BaseKitSelectionDialog({
  currentBaseKit,
  onBaseKitSelect,
  excludeKitId,
}: BaseKitSelectionDialogProps) {
  ...
  const handleClear = () => {
    setSelectedBaseKit(null);
  };

  const handleSubmit = () => {
    onBaseKitSelect(selectedBaseKit);
    setOpen(false);
  };

  const handleCancel = () => {
    setSelectedBaseKit(currentBaseKit);
    setSearchTerm("");
    setSearchResults([]);
    setOpen(false);
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" className="w-full justify-start">
          {currentBaseKit ? currentBaseKit.name : "Select Base Kit"}
        </Button>
      </DialogTrigger>
                ...
                {!selectedBaseKit ? (
                  ...
                  <div className="p-4">
                    <div className="space-y-3">
                      <KitSelectionCard kit={selectedBaseKit} />
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={handleClear}
