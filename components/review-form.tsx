...
export function ReviewForm({
  ...
  const overallScore =
    Math.round(
      (formData.scores.reduce((sum, score) => sum + score.score, 0) /
        formData.scores.length) *
        10
    ) / 10;
  ...
  const validateForm = (): boolean => {
    ...
    // Validate scores
    const scoreErrors: Record<ReviewCategory, string> = {} as Record<
      ReviewCategory,
      string
    >;
    ...
    formData.scores.forEach((score) => {
      if (!validateScore(score.score)) {
        scoreErrors[
          score.category
        ] = `Score must be between ${SCORE_CONSTRAINTS.MIN} and ${SCORE_CONSTRAINTS.MAX}`;
        hasScoreErrors = true;
      }
    });
                ...
                {formData.scores.map((scoreData) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        <label
                          htmlFor={`notes-${scoreData.category}`}
