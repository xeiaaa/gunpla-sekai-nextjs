...
export function KitEditForm({ kit }: KitEditFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    releaseDate: kit.releaseDate
      ? new Date(kit.releaseDate).toISOString().split("T")[0]
      : "",
  ...
  const handleMobileSuitsSelect = (
    selectedMobileSuits: typeof kit.mobileSuits
  ) => {
  ...
  const handleExpansionsSelect = (
    selectedExpansions: typeof kit.expansions
  ) => {
    setExpansions(selectedExpansions);
  };

  const handleExpandedBySelect = (
    selectedExpandedBy: typeof kit.expandedBy
  ) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setMessage({
          type: "error",
          text: result.error || "Failed to update kit",
        });
      }
    } catch (error) {
