...
interface ExpansionsSelectionDialogProps {
  currentExpansions: Kit[];
  onExpansionsSelect: (expansions: Kit[]) => void;
  excludeKitId?: string; // Kit ID to exclude from search results (current kit)
}

export function ExpansionsSelectionDialog({
  currentExpansions,
  onExpansionsSelect,
  excludeKitId,
}: ExpansionsSelectionDialogProps) {
  const [open, setOpen] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  const [searchResults, setSearchResults] = useState<Kit[]>([]);
  const [selectedExpansions, setSelectedExpansions] = useState<Kit[]>(currentExpansions);
  ...
  const handleRemoveExpansion = (kit: Kit) => {
    setSelectedExpansions((prev) => prev.filter((k) => k.id !== kit.id));
  };

  const handleSubmit = () => {
    onExpansionsSelect(selectedExpansions);
    setOpen(false);
  };

  const handleCancel = () => {
    setSelectedExpansions(currentExpansions);
    setSearchTerm("");
    setSearchResults([]);
    setOpen(false);
  };
      ...
      <DialogTrigger asChild>
        <Button variant="outline" className="w-full justify-start">
          {currentExpansions.length > 0
            ? `${currentExpansions.length} Expansion(s) Selected`
            : "Select Expansions"}
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Manage Kit Expansions</DialogTitle>
          <DialogDescription>
            Search and select expansion kits for this kit. Multiple kits can be selected.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 flex-1 flex flex-col min-h-0">
          ...
          <div className="flex-1 flex gap-4 min-h-0">
                    ...
                    {searchResults.map((kit) => (
                      ...
                      >
                        <div className={cn(
                          ...
                          selectedExpansions.some((k) => k.id === kit.id) && "ring-2 ring-primary ring-offset-2"
                        )}>
                          <KitSelectionCard kit={kit} />
                          {selectedExpansions.some((k) => k.id === kit.id) && (
            ...
            <div className="w-80 flex flex-col min-h-0">
              <Label>Selected Expansions ({selectedExpansions.length})</Label>
              <div className="flex-1 overflow-y-auto border rounded-md min-h-0">
                    ...
                    {selectedExpansions.map((kit) => (
                      ...
                      >
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleRemoveExpansion(kit)}
                        >
                          <X className="h-4 w-4" />
                        </Button>
                      </div>
