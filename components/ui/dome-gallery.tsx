...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(entries => {
      ...
      if (enlargedOverlay && frameRef.current && mainRef.current) {
        ...
        const img = enlargedOverlay.querySelector('img') as HTMLImageElement;

        if (img && img.complete && img.naturalWidth > 0) {
          const maxWidth = parseInt(openedImageWidth) || frameR.width;
          const maxHeight = parseInt(openedImageHeight) || frameR.height;
          ...
          // Fallback to original behavior if image not loaded
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    // Trigger the dimension calculation after the initial transform
