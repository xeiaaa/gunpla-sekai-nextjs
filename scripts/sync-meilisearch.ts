...
async function configureIndexes() {
  ...
  await meilisearch.index(INDEXES.mobileSuits).updateSettings({
    ...
    filterableAttributes: ["series.id", "series.timeline.id"],
  ...
  await meilisearch.index(INDEXES.series).updateSettings({
    ...
    filterableAttributes: ["timeline.id"],
    sortableAttributes: ["name", "createdAt"],
  });
  ...
  await meilisearch.index(INDEXES.users).updateSettings({
    ...
    sortableAttributes: ["username", "firstName", "lastName", "createdAt"],
  });
  ...
  await meilisearch.index(INDEXES.builds).updateSettings({
    ...
    filterableAttributes: ["user.id", "kit.id", "status"],
    sortableAttributes: ["title", "createdAt", "updatedAt"],
  });
  ...
  await meilisearch.index(INDEXES.reviews).updateSettings({
    ...
    filterableAttributes: ["user.id", "kit.id", "overallScore"],
    sortableAttributes: ["overallScore", "createdAt", "updatedAt"],
  });
...
async function syncMobileSuits() {
  ...
  const searchableMobileSuits: SearchableMobileSuit[] = mobileSuits.map(
    (ms) => ({
      id: ms.id,
      name: ms.name,
      slug: ms.slug,
  ...
  await meilisearch
    .index(INDEXES.mobileSuits)
    .addDocuments(searchableMobileSuits);
...
async function syncSeries() {
  ...
  const searchableSeries: SearchableSeries[] = series.map((s) => ({
    ...
    timeline: s.timeline
      ...
      : null,
    searchableText: [s.name, s.description, s.timeline?.name]
      .filter(Boolean)
      .join(" "),
  }));
...
async function syncProductLines() {
  ...
  const searchableProductLines: SearchableProductLine[] = productLines.map(
    (pl) => ({
      id: pl.id,
      name: pl.name,
      slug: pl.slug,
      ...
      searchableText: [pl.name, pl.description, pl.grade.name]
        .filter(Boolean)
        .join(" "),
    })
  );
  ...
  await meilisearch
    .index(INDEXES.productLines)
    .addDocuments(searchableProductLines);
...
async function syncGrades() {
  ...
  const searchableGrades: SearchableGrade[] = grades.map((g) => ({
    ...
    searchableText: [g.name, g.description].filter(Boolean).join(" "),
  }));
...
async function syncUsers() {
  ...
  const searchableUsers: SearchableUser[] = users.map((u) => ({
    ...
    searchableText: [u.username, u.firstName, u.lastName, u.email, u.bio]
      .filter(Boolean)
      .join(" "),
  }));
