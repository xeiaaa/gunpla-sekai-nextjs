...
import { getFilteredKitsWithMeilisearch, getFilterDataWithMeilisearch } from '../lib/actions/meilisearch-kits';
...
async function testKitsPageMeilisearch() {
  try {
    ...
    console.log(`   Mobile Suits: ${filterData.mobileSuits.length}`);
    ...
    console.log(`✅ Found ${basicResults.kits.length} kits (total: ${basicResults.total})`);
    ...
    console.log(`✅ Found ${variantResults.kits.length} kits (total: ${variantResults.total})`);
    ...
    const hgGrade = filterData.grades.find(g => g.name.toLowerCase().includes('high grade'));
    if (hgGrade) {
      ...
      console.log(`✅ Found ${gradeResults.kits.length} HG kits (total: ${gradeResults.total})`);
    ...
    if (filterData.productLines.length > 0) {
      ...
      console.log(`✅ Found ${productLineResults.kits.length} kits in ${filterData.productLines[0].name} (total: ${productLineResults.total})`);
    ...
    if (hgGrade) {
      ...
      console.log(`✅ Found ${combinedResults.kits.length} RX-78 HG kits (total: ${combinedResults.total})`);
