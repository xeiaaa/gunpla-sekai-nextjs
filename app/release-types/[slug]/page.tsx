...
import { getReleaseTypeBySlug, getReleaseTypeKits, getReleaseTypeAnalytics } from "@/lib/actions/release-types";
...
export async function generateMetadata({ params }: ReleaseTypeDetailPageProps) {
  const releaseType = await getReleaseTypeBySlug(params.slug);
...
export default async function ReleaseTypeDetail({ params }: ReleaseTypeDetailPageProps) {
  const releaseType = await getReleaseTypeBySlug(params.slug);
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {analytics.map(({ gradeName, count }) => (
                <div key={gradeName} className="bg-card border rounded-lg p-4 text-center">
                  <div className="text-2xl font-bold text-primary">{count}</div>
                  <div className="text-sm text-muted-foreground">{gradeName}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {recentKits.length === 0 ? (
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              <p className="text-muted-foreground text-lg">No kits found for {releaseType.name}</p>
          ...
          <>
            <div className="mb-6">
              <h2 className="text-2xl font-semibold mb-2">{releaseType.name} Kits</h2>
