...
export default function AddKitsToMobileSuitsPage() {
  ...
  const filteredKits = kits.filter(
    (kit) =>
      ...
      (kit.productLine?.name
        .toLowerCase()
        .includes(kitSearchTerm.toLowerCase()) ??
        false) ||
      (kit.series?.name.toLowerCase().includes(kitSearchTerm.toLowerCase()) ??
        false) ||
      (kit.grade?.toLowerCase().includes(kitSearchTerm.toLowerCase()) ??
        false) ||
      kit.mobileSuits.some((ms) =>
        ms.toLowerCase().includes(kitSearchTerm.toLowerCase())
      )
  );
  ...
  const filteredMobileSuits = mobileSuits.filter(
    (ms) =>
      ...
      (ms.description
        ?.toLowerCase()
        .includes(mobileSuitSearchTerm.toLowerCase()) ??
        false) ||
      (ms.series?.name
        .toLowerCase()
        .includes(mobileSuitSearchTerm.toLowerCase()) ??
  ...
  const handleAddRelationships = async () => {
    if (selectedKits.length === 0 || selectedMobileSuits.length === 0) {
      setMessage({
        type: "error",
        text: "Please select at least one kit and one mobile suit",
      });
      return;
    }
    ...
    try {
      const result = await addKitToMobileSuits(
        selectedKits,
        selectedMobileSuits
      );
      ...
      if (result.success) {
        ...
        setMessage({
          type: "error",
          text: result.error || "Failed to add relationships",
        });
      }
    } catch (error) {
  ...
  const handleRemoveRelationships = async () => {
    ...
    try {
      const result = await removeKitFromMobileSuits(
        selectedKits,
        selectedMobileSuits
      );
      ...
      if (result.success) {
        ...
        setMessage({
          type: "error",
          text: result.error || "Failed to remove relationships",
        });
      }
    } catch (error) {
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Select kits and mobile suits to create many-to-many relationships
          between them.
        </p>
      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-muted-foreground">
                            {mobileSuit.kitsCount} kit
                            {mobileSuit.kitsCount !== 1 ? "s" : ""}
                            {mobileSuit.series &&
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Add or remove relationships between the selected kits and mobile
            suits
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
              {updating
                ? "Adding..."
                : `Add ${selectedKits.length} kit${
                    selectedKits.length !== 1 ? "s" : ""
                  } to ${selectedMobileSuits.length} mobile suit${
            ...
            >
              {updating
                ? "Removing..."
                : `Remove relationships between ${selectedKits.length} kit${
                    selectedKits.length !== 1 ? "s" : ""
                  } and ${selectedMobileSuits.length} mobile suit${
                    selectedMobileSuits.length !== 1 ? "s" : ""
                  }`}
            </Button>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {selectedMobileSuits.length > 0 && (
                <div className="p-3 bg-green-50 rounded-lg">
                  <h4 className="font-medium mb-2">
                    Selected Mobile Suits ({selectedMobileSuits.length}):
                  </h4>
                  <ul className="text-sm text-muted-foreground space-y-1">
