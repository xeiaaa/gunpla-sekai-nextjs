...
export async function generateMetadata({ params }: TimelineDetailPageProps) {
  ...
  return {
    ...
    description: timeline.description || `Explore ${timeline.name} timeline and its series`,
  };
}

export default async function TimelineDetail({ params }: TimelineDetailPageProps) {
  const timeline = await getTimelineBySlug(params.slug);
      ...
      <div className="bg-primary text-primary-foreground py-8">
        <div className="container mx-auto px-4">
          ...
          {timeline.description && (
            <p className="text-lg opacity-90 max-w-3xl">{timeline.description}</p>
          )}
        ...
        {timeline.series.length === 0 ? (
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              <p className="text-muted-foreground text-lg">No series found in this timeline</p>
          ...
          <>
            <div className="mb-6">
              <h2 className="text-2xl font-semibold mb-2">Series in {timeline.name}</h2>
