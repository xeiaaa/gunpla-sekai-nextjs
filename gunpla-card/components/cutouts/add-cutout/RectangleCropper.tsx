...
const RectangleCropper = forwardRef<
  RectangleCropperHandle,
  { sourceUrl: string }
>(({ sourceUrl }, ref) => {
  ...
  useEffect(() => {
    ...
    setRect({
      x: Math.floor((display.width - w) / 2),
      y: Math.floor((display.height - h) / 2),
      width: w,
      height: h,
    });
  }, [display]);
  ...
  useImperativeHandle(
    ...
    () => ({
      async getDataUrl() {
        ...
        const sw = Math.min(
          imgEl.naturalWidth - sx,
          Math.floor(rect.width * scale)
        );
        const sh = Math.min(
          imgEl.naturalHeight - sy,
      ...
      <Stage width={display.width} height={display.height}>
        <Layer>
          ...
          <Rect
            ...
            draggable
            onDragEnd={(e) =>
              setRect((r) => ({ ...r, x: e.target.x(), y: e.target.y() }))
            }
            onTransformEnd={() => {
