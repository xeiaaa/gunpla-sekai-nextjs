...
const loadImageEl = (src: string): Promise<HTMLImageElement> =>
  new Promise((resolve) => {
    ...
    img.src = src.startsWith("http")
      ? `/api/gunpla-card/proxy-image?url=${encodeURIComponent(src)}`
      : src;
  });

const CirclePolygonCropper = forwardRef<
  CirclePolygonCropperHandle,
  {
    sourceUrl: string;
    mode: ShapeMode;
...
>(({ sourceUrl, mode, onPolygonClosedChange }, ref) => {
  ...
  const proxied = useMemo(
    () =>
      sourceUrl.startsWith("http")
        ? `/api/gunpla-card/proxy-image?url=${encodeURIComponent(sourceUrl)}`
        : sourceUrl,
    [sourceUrl]
  );
  ...
  useEffect(() => {
    ...
    setCircle({
      cx: display.width / 2,
      cy: display.height / 2,
      r: Math.min(display.width, display.height) / 5,
    });
  }, [mode, display]);
        ...
        <Layer>
          ...
          {mode === "circle" && circle ? (
            <KonvaCircle
              ...
              draggable
              onDragMove={(e) =>
                setCircle({ ...circle, cx: e.target.x(), cy: e.target.y() })
              }
            />
          ) : null}
          {mode === "polygon" && points.length > 0 ? (
            <Group>
              <Line
                ...
                strokeWidth={2}
                closed={closed}
                fill={closed ? "rgba(0,255,255,0.2)" : undefined}
              />
              {points
                .reduce<{ x: number; y: number; i: number }[]>(
                  (acc, _v, idx) => {
                    if (idx % 2 === 0)
                      acc.push({ x: points[idx], y: points[idx + 1], i: idx });
                    return acc;
                  },
                  []
                )
